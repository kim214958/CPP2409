...
int main()
{
	...
	{ // 사용자에게 계속 입력받기 위해 무한 루프
		...
		cout << "명령어를 입력하세요 (up,down,left,right,map,exit,info): ";
		cin >> user_input;

		if (user_input == "up")
		{
			correctmove = nextpoint(x, y, 0, -1, map); // 이동하는 함수, 유효한 이동이면 true를 correctmove에 반환
			if (correctmove)
				checkEvent(map, currentPlayer, x, y); // 유효한 이동이면 event가 있는지 확인&발생
		}
		else if (user_input == "down")
		{
			correctmove = nextpoint(x, y, 0, 1, map); // 위와 동
			if (correctmove)
				checkEvent(map, currentPlayer, x, y); // 위와 동
		}
		else if (user_input == "left")
		{
			correctmove = nextpoint(x, y, -1, 0, map); // 위와 동
			if (correctmove)
				checkEvent(map, currentPlayer, x, y); // 위와 동
		}
		...
		if (checkGoal(map, x, y)) {
            cout << (current_turn == 0 ? "Magician" : "Warrior") << "이(가) 목적지에 도착했습니다! 축하합니다!" << endl;
            break;
        }
		...
		if (!CheckUser(currentPlayer) || currentPlayer.GetHP() <= 0)
		{ // CheckUser(user)가 false (user.hp == 0이면 false를 반환)이거나, user.GetHP()의 반환값 hp가 음수이면 if문을 실행
			cout << (current_turn == 0 ? "Magician" : "Warrior") << "의 HP가 0 이하가 되었습니다. 실패했습니다." << endl;
			break;
		}
		// 남은 체력을 표시해줌
		else
			cout << "현재 HP: " << currentPlayer.GetHP() << " "; // user.GetHP()의 반환값 user.hp를 표시
	}
	// 반복문이 종료되면 프로그램 종료
	return 0;
}
